...
static inline double entropy(unsigned char byte_count[], int length)
{
  double val = 0.0;
  int i;
  ...
  /* entropy calculation */
  for (i = 0; i < length; i++) 
    val += (double) byte_count[i] / (double) length;
